...
tree = readRDS("~/hpc/pmc_vanboxtel/projects/Burkitt/3_Output/TreeBuilding_Alex/P856/Filtered_samples_v2/CPW_04/TreeObject0.4.RDS")
vcf = VariantAnnotation::readVcf("~/hpc/pmc_vanboxtel/projects/Burkitt/3_Output/TreeBuilding_Alex/P856/Filtered_samples_v2/P856.vep.sub.SNV.autosomal.noBulksNoBlacklist.vcf")
...
vcf_bulk_filtered_BM <- vcf_bulk_autosomal[vaf_values_BM > 0.30] # filter on VAF (cutoff decided in previous script now called Mutational_load_VAF_cutoff)
vcf_bulk_filtered_PL <- vcf_bulk_autosomal[vaf_values_PL > 0.30] # filter on VAF (cutoff decided in previous script now called Mutational_load_VAF_cutoff)
