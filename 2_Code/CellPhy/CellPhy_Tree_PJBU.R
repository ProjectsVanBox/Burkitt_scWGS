...
# Description: Script to look at CellPhy Tree of Donor PJBU (branch VCFs for manual checking, choosing percentage, signatures per branch)
...
tree = readRDS("~/hpc/pmc_vanboxtel/projects/Burkitt/3_Output/TreeBuilding_Alex/PJBU/Filtered_samples_v3/CPW_04/TreeObject0.4.RDS")
...
vcf_bulk = VariantAnnotation::readVcf("~/hpc/pmc_vanboxtel/projects/Burkitt/1_Input/ASAP_FROM_CLOUD/PJBU/vcf_batches/batch_bulk/vcf/germline/somatic_filtering/SMuRF/PJBU_bulk.vep.SMuRF.filtered.sorted.vcf.gz")
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PJBU_absolute_tree_with_branch_info.pdf",
  plot = p1,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PJBU_absolute_tree.pdf",
  plot = p2,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PJBU_absolute_tree_with_bootstraps.pdf",
  plot = p2b,
  width = 8,
  height = 6
)
...
corrected_tree <- correct_branches(tree, input_df_PJBU_filtered)
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PJBU_absolute_tree_corrected_for_callable_loci.pdf",
  plot = p3,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PJBU_absolute_tree_with_bootstraps_corrected_for_callable_loci.pdf",
  plot = p3b,
  width = 8,
  height = 6
)
...
vaf_matrix <- vaf_matrix[, 1, drop = FALSE]  # keep as a matrix
...
bulk_vaf_matrix <- geno(vcf_bulk_autosomal)$VAF
...
group1 <- c("m2")
group2 <- c("T", "l2","j2","T2","S", "Q","L")
group3 <- c("k", "j", "m", "i", "p", "h", "s", "f", "e", "d", "w", "c", "Z", "Y", "W","V", "G2", "F2", "E2", "D2", "K2", "M2", "O2", "C2", "B2", "A2", "U2", "W2", "b2", "a2", "Z2", "f2", "e2", "Y2", "k2", "N", "M", "P", "R", "K", "J", "I")
group4 <- c("p2", "o2", "r2", "t2", "v2", "n2", "A3", "z2", "C3", "G3", "F3", "I3", "G", "F", "D", "A")
...
plot_df <- rbind(
  ...
  data.frame(VAF = vafs_group2, Group = "BL-Intermediate"),
  data.frame(VAF = vafs_group3, Group = "BL-Private"),
  data.frame(VAF = vafs_group4, Group = "WT")
)
...
summary_stats <- rbind(
  ...
  cbind(get_shared_vs_total_stats(group2, shared_stats, tree_mut_counts), Group = "BL-Intermediate"),
  cbind(get_shared_vs_total_stats(group3, shared_stats, tree_mut_counts), Group = "BL-Private"),
  cbind(get_shared_vs_total_stats(group4, shared_stats, tree_mut_counts), Group = "WT")
)

plot_df$Group <- factor(plot_df$Group, levels = c("BL-Trunk", "BL-Intermediate", "BL-Private", "WT"))
summary_stats$Group <- factor(summary_stats$Group, levels = c("BL-Trunk", "BL-Intermediate", "BL-Private", "WT"))

group_cols <- c(
  "BL-Trunk" = "#1D3557",
  "BL-Intermediate" = "#0A9086",
  "BL-Private" = "#A62639",
  "WT" = "#D2BD96"
)


p4 <- ggplot(plot_df, aes(x = VAF)) +
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PJBU_bulk_VAF_distribution_histogram.pdf",
  plot = p4,
  width = 8,
  height = 6
)

p5 <- ggplot(plot_df, aes(y = VAF)) +
  ...
  geom_text(
    data = summary_stats,
    aes(x = 1, y = Inf, hjust = 1.05, vjust = 1.4, label = Label),
    inherit.aes = FALSE,
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PJBU_bulk_VAF_distribution_violin.pdf",
  plot = p5,
  width = 8,
  height = 6
)
...
p6 <- ggplot(intermediate_df %>% group_by(Branch) %>% mutate(nuniq = n_distinct(VAF)) %>% ungroup(), aes(x = 1, y = VAF)) +
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PJBU_bulk_VAF_distribution_intermediate_branches_violin.pdf",
  plot = p6,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PJBU_reconstructed_cosine_without_PTA.pdf",
  plot = p7,
  width = 8,
  height = 6
)

# missing some signature(s) so will include PTA artifact signature
...
# Save as PDF
#ggsave(
#  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PJBU_reconstructed_cosine_with_PTA.pdf",
#  plot = p8,
#  width = 8,
#  height = 6
#)

# all branches >200 mutations are above the 0.85 threshold so keep PTA signature (no longer applicable)
...
p9 <- plot_tree_contribution_bars_new_alex(
  ...
  branch_color = "#000000",
  x_limits = c(0, 4000),
  x_breaks = seq(0, 4000, by = 200)
)

# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PJBU_signatures_in_tree.pdf",
  plot = p9,
  width = 8,
  height = 6
)
...
column_names %in% group1 ~ "BL-Trunk",
  column_names %in% group2 ~ "BL-Intermediate",
  column_names %in% group3 ~ "BL-Private",
  column_names %in% group4 ~ "WT"
)
...
#  mutate(
#    BL_post_expansion = coalesce(`BL-Intermediate`, 0) + coalesce(`BL-Private`, 0)
#  )

# Re-order

contribution_wide <- contribution_wide |>
  dplyr::select(sample, WT, `BL-Trunk`, `BL-Intermediate`, `BL-Private`)
...
contribution_matrix
write.csv(contribution_matrix, "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Data/sbs_contr_per_group_PJBU.csv", row.names = T)

p10 <- plot_contribution(contribution_matrix[,], palette = sig_cols) # All signatures including PTA

# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PJBU_signatures_per_group_with_PTA.pdf",
  plot = p10,
  width = 8,
  height = 6
)
...
# Save as PDF
#ggsave(
#  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PJBU_signatures_per_group_without_PTA.pdf",
#  plot = p11,
#  width = 8,
#  height = 6
#)

#p11_sub <- plot_contribution(contribution_matrix[2:6,3:5], palette = sig_cols) # without PTA

# Save as PDF
#ggsave(
#  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PJBU_signatures_per_group_without_PTA_sub.pdf",
#  plot = p11_sub,
#  width = 8,
#  height = 6
#)

#p11a_sub <- plot_contribution(contribution_matrix[2:6, c("BL-Trunk", "BL_post_expansion")], palette = sig_cols) # without PTA

# Save as PDF
#ggsave(
#  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PJBU_signatures_pre_vs_post_expansion_without_PTA_sub.pdf",
#  plot = p11a_sub,
#  width = 8,
#  height = 6
#)
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PJBU_signatures_per_group_without_PTA_separate_bars.pdf",
  plot = p11b,
  width = 8,
  height = 6
)

p11b_sub <- ggplot(transform(subset(df_long, Sample %in% c("BL-Trunk","BL-Intermediate","BL-Private")), Sample = factor(Sample, levels = c("BL-Trunk","BL-Intermediate","BL-Private"))), aes(x = Sample, y = Prop, fill = Signature)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = sig_cols) +
  scale_y_continuous(limits = c(0, 1)) +     # y-axis from 0 to 1
  labs(title = "Signature contribution per sample (no PTA)",
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PJBU_signatures_per_group_without_PTA_separate_bars_sub.pdf",
  plot = p11b_sub,
  width = 8,
  height = 6
)
...
sample_groups <- c(
  ...
  setNames(rep("BL-Intermediate", length(group2)), group2),
  setNames(rep("BL-Private", length(group3)), group3),
  setNames(rep("WT", length(group4)), group4)
)

# Add merged BL group
BL_samples <- c(group1, group2, group3)
sample_groups <- c(sample_groups, setNames(rep("BL", length(BL_samples)), BL_samples))

# Subset matrix to only relevant samples

branch_sel <- branch_mm[, names(sample_groups), drop = FALSE]
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PJBU_CTpG_fraction_custom_groups.pdf",
  plot = p11c,
  width = 6,
  height = 4.5
)
...
agedf <- data.frame(
  ...
  age = rep(6.6,116)  # Replace these with your actual ages
)
...
p12 <- plot_tree_contribution_bars_new_alex(
  ...
  title = "PJBU", bar_height = 0.02, scaling = 0.8, add_branch_length = F,
  branch_color = "#000000",
  x_limits = c(0, 3600),
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PJBU_signatures_in_tree_without_PTA_callable_loci_corrected.pdf",
  plot = p12,
  width = 8,
  height = 6
)
...
tip_data <- input_df_PJBU %>%
  mutate(color_group = ifelse(Myc_translocation_IGV == "Yes", "#4378bd", "#e7872b"))

p13 <- plot_gg_tree(tree,
                    add_branch_length = FALSE,
                    add_bootstrap = FALSE,
                    add_tip_label = FALSE,
                    add_title = "") %<+% tip_data +   # attach annotation to tree
  geom_tree(color = "grey") +
  geom_tippoint(aes(color = Myc_translocation_IGV), size = 3) +  # map color to Yes/No
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PJBU_absolute_tree_without_PTA_corrected_for_callable_loci.pdf",
  plot = p13,
  width = 8,
  height = 6
)
...
# get only SBSblood to do ultrametric?

sbs_constant_only <- colSums(sig_contri[c("SBSblood", "SBS1"), ])
names(sbs_constant_only) <- colnames(sig_contri)
missing_branches <- setdiff(x$branch_id, names(sbs_constant_only))
sbs_constant_only[missing_branches] <- 0
x$branch_length_constant_only <- ifelse(names(sbs_constant_only) %in% x$branch_id, 
                                        sbs_constant_only[x$branch_id], 
                                    0)
x$branch_length_constant_only <- as.integer(x$branch_length_constant_only)

phylo_tree$edge.length<-unname(x$branch_length_constant_only)
phylo_tree$edge
print(phylo_tree)
# Create the age dataframe
agedf <- data.frame(
...
x <- x %>%
  mutate(product_sensitivity = sapply(samples, calculate_product_sensitivity, sensitivity_df = input_df_PJBU_filtered),
         corr_branch_lengths = x$branch_length_constant_only / product_sensitivity)
