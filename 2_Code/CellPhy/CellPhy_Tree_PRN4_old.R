...
vcf_sc = readVcf("~/hpc/pmc_vanboxtel/projects/Burkitt/3_Output/TreeBuilding_Alex/PRN4/Filtered_samples_v2/PRN4.vep.sub.SNV.autosomal.noBulksNoBlacklist.vcf")
...
# VCF per branch
branch_vcf = extract_vcf_per_branch(tree = tree, vcf = vcf_sc, ref_genome = ref_genome)
...
vcf_bulk_snvs <- vcf_bulk_LN[isSNV(vcf_bulk_LN)] # only SNVs
...
vcf_bulk_filtered_LN <- vcf_bulk_autosomal[vaf_values_LN > 0] # filter on VAF (cutoff decided in previous script called Mutational_load_VAF_cutoff)

seqlevels(vcf_bulk_BM) <- gsub("^chr", "", seqlevels(vcf_bulk_BM))
tumor_sample <- "PMABM000FZO"
bulk_df_BM <- data.frame()

if (tumor_sample %in% samples(header(vcf_bulk_BM))) {
  ad <- sapply(geno(vcf_bulk_BM)$AD[, tumor_sample], "[[", 2)  # Alt depth
  dp <- geno(vcf_bulk_BM)$DP[, tumor_sample]  # Total depth
  vaf <- ad / dp
  
  bulk_df_BM <- data.frame(variant = names(vaf), vaf = as.numeric(vaf), bulk = tumor_sample)
}
bulk_df_BM <- bulk_df_BM %>%
  filter(!str_detect(variant, "chrX|chrY"))
bulk_df_BM_filtered <- bulk_df_BM %>%
  filter(vaf >= 0.27)
hist(bulk_df_BM_filtered$vaf,
     xlim = c(0, 1),
     breaks = 10,
     col = "skyblue",
     border = "white",
...
ggplot(bulk_df_BM_filtered, aes(x = "", y = vaf)) +
  coord_cartesian(ylim = c(0, 1)) +
  geom_hline(yintercept = 0.27) +
  geom_violin(fill = "skyblue") +
  labs(title = "Violin Plot of VAF", y = "VAF", x = "") +
  theme_minimal()

median_vaf <- median(bulk_df_BM_filtered$vaf, na.rm = TRUE)
print(median_vaf)
...
# Now plot each group separately but within each group show all the different branches individually 

get_branch_annotation_df <- function(branches, stats_list, tree_df) {
  do.call(rbind, lapply(branches, function(branch) {
    shared <- if (!is.null(stats_list[[branch]])) length(stats_list[[branch]]$shared_vafs) else 0
    total <- tree_df$total_muts[match(branch, tree_df$branch)]
    
    if (is.na(total)) total <- 0
    
    label <- if (total > 0) {
      paste0(shared, "/", total, " = ", round(100 * shared / total, 1), "%")
    } else {
      paste0(shared, "/0 = NA")
    }
...
summary_df1 <- get_branch_annotation_df(group1, shared_stats, tree_mut_counts)
summary_df2 <- get_branch_annotation_df(group2, shared_stats, tree_mut_counts)
summary_df3 <- get_branch_annotation_df(group3, shared_stats, tree_mut_counts)

build_vaf_df_by_branch <- function(branches, group_label, stats_list) {
  do.call(rbind, lapply(branches, function(branch) {
    if (!is.null(stats_list[[branch]]) && length(stats_list[[branch]]$shared_vafs) > 0) {
      data.frame(
        VAF = stats_list[[branch]]$shared_vafs,
        Branch = branch,
        Group = group_label
      )
    } else {
...
plot_df1 <- build_vaf_df_by_branch(group1, "Trunk", shared_stats)
plot_df2 <- build_vaf_df_by_branch(group2, "Intermediate", shared_stats)
plot_df3 <- build_vaf_df_by_branch(group3, "Private", shared_stats)
...
ggplot(plot_df1, aes(x = VAF)) +
  ...
  labs(
    title = "Bulk VAF Distributions - Trunk Branches",
    x = "Bulk VAF",
    y = "Count"
  )
...
ggplot(plot_df2, aes(x = VAF)) +
  ...
  theme_classic() +
  labs(
    title = "Bulk VAF Distributions - Intermediate Branches",
    x = "Bulk VAF",
    y = "Count"
  )
...
ggplot(plot_df3, aes(x = VAF)) +
  ...
  theme_classic() +
  labs(
    title = "Bulk VAF Distributions - Private Branches",
    x = "Bulk VAF",
    y = "Count"
  )
