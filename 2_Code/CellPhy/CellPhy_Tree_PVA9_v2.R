...
# Description: Script to look at CellPhy Tree, branch signatures and Ultrametric tree for Donor PVA9
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PVA9/PVA9_absolute_tree_with_branch_info.pdf",
  plot = p1,
  width = 8,
  height = 6
)
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PVA9/PVA9_absolute_tree.pdf",
  plot = p2,
  width = 8,
  height = 6
)
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PVA9/PVA9_absolute_tree_with_bootstraps.pdf",
  plot = p2b,
  width = 8,
  height = 6
)
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PVA9/PVA9_absolute_tree_corrected_for_callable_loci.pdf",
  plot = p3,
  width = 8,
  height = 6
)
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PVA9/PVA9_absolute_tree_with_bootstraps_corrected_for_callable_loci.pdf",
  plot = p3b,
  width = 8,
  height = 6
)
...
saveRDS(branch_vcf, file = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Data/branch_vcf_PVA9.rds")
branch_grl = convert_vcf_to_granges(branch_vcf_list = branch_vcf, ref_genome = ref_genome)
saveRDS(branch_grl, file = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Data/branch_grl_PVA9.rds")
...
group1 <- c("J3") # BL trunk
group2 <- c("H3", "G3","q2","N","v2", "E3") # BL intermediate
group3 <- c("O2", "N2", "Q2", "S2", "H2", "G2", "J2", "L2", "V2", "F2", "C2", "B2", "A2", "a2", "Z2", "z", "e2", "u", "t", "w", "r", "q", "o", "n","c", "b", "e", "h", "g", "k", "a", "Z", "l2","k2", "n2", "Q", "P", "O","T", "W", "V", "A3","z2", "x2", "w2", "D3", "s2", "r2","u2", "M", "L", "I3") # BL private
group4 <-  c("L3", "K3", "N3", "K", "G", "F", "I", "T3", "S3", "B", "A") # WT
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PVA9/PVA9_bulk_VAF_distribution_histogram.pdf",
  plot = p4,
  width = 8,
  height = 6
)
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PVA9/PVA9_bulk_VAF_distribution_violin.pdf",
  plot = p5,
  width = 8,
  height = 6
)
...
# Per-branch shared vs total (with %) for Intermediate branches

branches <- names(intermediate_list)

shared_counts <- sapply(branches, function(b) {
  vals <- intermediate_list[[b]]$shared_vafs
  if (is.null(vals)) 0 else sum(!is.na(vals))
})

totals <- tree_mut_counts$total_muts[match(branches, tree_mut_counts$branch)]

pct <- ifelse(is.na(totals) | totals == 0, NA_real_, 100 * shared_counts / totals)

labels <- ifelse(
  is.na(pct),
  paste0(shared_counts, "/", ifelse(is.na(totals) | totals == 0, "?", totals)),
  paste0(shared_counts, "/", totals, " = ", round(pct, 1), "%")
)

branch_summary <- data.frame(
  Branch = branches,
  x = 1,            # same x-position as the violins
  y = Inf,          # place at the top of each facet
  Label = labels,
...
p6 <- ggplot(intermediate_df %>% group_by(Branch) %>% mutate(nuniq = n_distinct(VAF)) %>% ungroup(), aes(x = 1, y = VAF)) +
  ...
  inherit.aes = FALSE,
  vjust = 1.4, hjust = 1.05, size = 3, fontface = "bold"
  )

# Save as PDF

ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/CellPhy/Figures/PVA9/PVA9_bulk_VAF_distribution_intermediate_branches_violin.pdf",
  plot = p6,
  width = 8,
  height = 6
)
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PVA9/PVA9_reconstructed_cosine_per_branch.pdf",
  plot = p7,
  width = 8,
  height = 6
)

write.csv(contribution, file = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Data/Trees/PVA9_per_branch_sbs_contribution.csv", row.names = TRUE)
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PVA9/PVA9_signatures_in_tree.pdf",
  plot = p9,
  width = 8,
  height = 6
)
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PVA9/PVA9_signatures_per_group.pdf",
  plot = p10,
  width = 8,
  height = 6
)
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Trees/PVA9/PVA9_signatures_per_group_separate_bars.pdf",
  plot = p11,
  width = 8,
  height = 6
)
