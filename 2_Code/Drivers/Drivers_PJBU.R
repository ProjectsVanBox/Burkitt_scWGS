...
# folder containing the PJBU batch VCFs

vcf_dir <- "~/hpc/pmc_vanboxtel/projects/Burkitt/2_Code/Drivers/Markus/PJBU/Single_cell_drivers"

# find all batches (handles both underscore and dot separators)

vcf_files <- list.files(
  vcf_dir,
  #pattern = "^PJBU[._][0-9]+\\.vep\\.effect\\.genes\\.somatic\\.pass\\.vcf$",
  #pattern = "^PJBU_[0-9]+\\.vep\\.effect_2\\.genes_2\\.somatic_2\\.pass_2\\.vcf$",
  pattern = "^PJBU_[0-9]+\\.vep\\.effect_3\\.somatic\\.pass\\.vcf$",
  full.names = TRUE
)

if (length(vcf_files) == 0) stop("No PJBU batch VCFs found in: ", vcf_dir)
...
# Filter out rows which do not have genes in the list of known genes
...
oncoplot_matrix <- oncoplot_matrix %>%
  dplyr::select(driver, all_of(PJBU_samples))
...
sample_group <- ifelse(ordered_samples %in% PJBU_samples_filtered, "Ascites", NA)
...
# Categorize samples in PJBU_rows
PJBU_rows <- PJBU_rows %>%
  mutate(
    Category = case_when(
...
#rownames(effect_matrix_after_check_T) <- stringr::str_trunc(
...
# Plot
pdf("~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/Drivers/Figures/PJBU_oncoprint_20250926.pdf",
    width = 10, height = 6)
...
samples_in_clade_wo_driver <- c(
  "PJBUGTDABC35", "PJBUGTDABC25", "PJBUGTDABC23", "PJBUGTDABC41",
  "PJBUGTDABC17", "PJBUGTDABC45", "PJBUGTDABC18", "PJBUGTDABC49",
  "PJBUGTDABC52", "PJBUGTDABC1",  "PJBUGTDABC53", "PJBUGTDABC44",
  "PJBUGTDABC38", "PJBUGTDABC39", "PJBUGTDABC21", "PJBUGTDABC58",
  "PJBUGTDABC55", "PJBUGTDABC16", "PJBUGTDABC2",  "PJBUGTDABC31",
  "PJBUGTDABC27", "PJBUGTDABC40", "PJBUGTDABC3",  "PJBUGTDABC32",
  "PJBUGTDABC56", "PJBUGTDABC51"
)
...
# Keep drivers: >=5 in clade AND 0 elsewhere

keep_rows <- (count_clade >= 5) & (count_other == 0)
...
sample_group_sub <- ifelse(ordered_subset %in% PJBU_samples_filtered, "Ascites", NA)
...
oncoPrint(
    ...
    alter_fun              = alter_fun,
    col                    = col,
    column_title           = "PJBU oncoplot (clade-only drivers ≥5)",
    heatmap_legend_param   = heatmap_legend_param,
    show_column_names      = TRUE,
