...
effect_matrix_after_check <- read.csv("~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/Drivers/P856_manual_driver_check.csv")
...
missing_samples <- setdiff(P856_samples, colnames(effect_matrix_after_check_T))
...
ordered_samples <- P856_samples[P856_samples %in% colnames(effect_matrix_after_check_T)]
all_samples <- colnames(effect_matrix_after_check_T)
sample_group <- ifelse(ordered_samples %in% P856_PL, "PL",
                       ifelse(ordered_samples %in% P856_BM, "BM", NA))

# Define group colors

group_colors <- c(PL = "grey", BM = "black") 
...
P856_Bulk <- c("PB14458-BLPL-BCELLBULK", "PB14458-BLBM-BCELLBULK")

# Define CellType annotation

Sample <- case_when(
  ordered_samples %in% normal_bcell_samples ~ "Normal B-cell",
  ordered_samples %in% P856_Bulk        ~ "Bulk Tumour",
...
annot_df <- data.frame(
  Location  = sample_group,   # BM / PL / NA
...
# Within-BL block, PL first then BM
...
pdf("~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/Drivers/P856_oncoprint_after_manual_check.pdf", width = 10, height = 6)  # adjust size as needed
