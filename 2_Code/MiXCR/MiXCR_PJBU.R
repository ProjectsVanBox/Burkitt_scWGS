...
PJBU_files <- list.files(path = "~/hpc/pmc_vanboxtel/projects/Burkitt/3_Output/MIXCR_new/PJBU/Output/.", pattern = "\\.tsv$", recursive = TRUE, full.names = TRUE)

# Read each .tsv file into a list

tsv_files_list <- lapply(PJBU_files, function(x) read.delim(x, header = TRUE, sep = "\t"))
...
write.table(combined_df, "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MIXCR/Data/PJBU_combined_df.txt")

# Possibility to continue from saved df

combined_df <- read.table("~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MIXCR/Data/PJBU_combined_df.txt")
...
pdf("Figures/PJBU_MiXCR_BCR_complete_heatmap.pdf", width=8, height=14)
Heatmap(t(bcr_mat),
        na_col="white",
        col=col_discrete,
        color_space = "RBG",
