...
# Save to CSV
write.csv(df_sbs9_status, "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Data/Denovo_inhouse_single_cell_Machado_pcawg/Normal_SBS9_status_table.csv", row.names = FALSE)
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Group_comparisons/SBS_high_vs_low_SNV_load_comparison_WT_only_all_samples_20251003.pdf",
...
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Group_comparisons/WT_ageline_per_sig_and_ctgpg_all_samples_20251003.pdf",
  plot = plot, width = 7, height = 2
)

# Get relative contribution of signatures per age
...
# BL vs WT (SBS9 pos/neg) mixed-effects EMMs 
...
# Parameters & order (WT-SBS9neg → WT-SBS9pos → BL-Trunk → BL-Intermediate → BL-Private)
blwt_sig_keep      <- c("SBS1", "SBS7a", "SBS17b","SBS18","SBS9","SBSblood")
blwt_keep_cols_bl  <- c("BL_Trunk","BL_Intermediate", "BL_Private")
blwt_keep_cols_all <- c("WT_SBS9_neg","WT_SBS9_pos","BL_Trunk","BL_Intermediate", "BL_Private")
blwt_threshold     <- 0  # Threshold to consider a cell SBS9+ i.e. more than 0% contribution of SBS9
...
blwt_wt_group <- ifelse(blwt_sbs9_prop[blwt_wt_samples] > blwt_threshold, "WT_SBS9_pos", "WT_SBS9_neg")
...
blwt_df_long_wt <- lapply(seq_along(blwt_wt_samples), function(i) {
  ...
  dplyr::mutate(
    Signature = factor(Signature, levels = blwt_sig_keep),
    Group     = factor(Group, levels = c("WT_SBS9_neg","WT_SBS9_pos"))
  )
...
# Beta-GLMM per signature
# We use a beta-GLMM because the outcome is a proportion (0–1), and the beta distribution is designed for modeling continuous proportions while respecting those boundaries. 
# Adding random effects (the “GLMM” part) accounts for repeated measures and patient-to-patient variability, so group comparisons reflect true differences rather than being dominated by uneven sampling.
...
for (sig in levels(blwt_df_beta$Signature)) {
  ...
  if (all(c("WT_SBS9_neg","WT_SBS9_pos") %in% levs)) {
    contr_defs[["WT SBS9 pos - WT SBS9 neg"]] <- wvec(c("WT_SBS9_pos"= 1, "WT_SBS9_neg"= -1))
  }
  if (all(c("WT_SBS9_pos","BL_Trunk") %in% levs)) {
    contr_defs[["WT SBS9 pos - BL Trunk"]] <- wvec(c("WT_SBS9_pos"= 1, "BL_Trunk"= -1))
  }
...
x_wt_pair        <- xpair("WT_SBS9_neg","WT_SBS9_pos")
...
annot_wt <- blwt_emm_all |>
  dplyr::filter(Group %in% c("WT_SBS9_neg","WT_SBS9_pos")) |>
  dplyr::group_by(Signature) |>
  dplyr::summarise(y_pos = pmin(1, max(upper, na.rm = TRUE) * 1.08), .groups = "drop") |>
  dplyr::mutate(x_pos = x_wt_pair, contrast = "WT SBS9 pos - WT SBS9 neg")

annot_cross <- blwt_emm_all |>
  dplyr::filter(Group %in% c("WT_SBS9_pos","BL_Trunk")) |>
  dplyr::group_by(Signature) |>
  dplyr::summarise(y_pos = pmin(1, max(upper, na.rm = TRUE) * 1.08), .groups = "drop") |>
  dplyr::mutate(x_pos = x_cross_pair, contrast = "WT SBS9 pos - BL Trunk")
...
p_blwt <- ggplot(blwt_emm_all, aes(x = Group, y = emmean, color = Signature)) +
  ...
  labs(
    x = NULL, y = "Estimated mean relative contribution (proportion)",
    title = "Estimated proportions by group (WT: SBS9-neg/pos; BL: Trunk/Intermediate/Private)"
  ) +
  ...
  geom_text(data = blwt_annot, aes(x = x_pos, y = y_pos, label = stars),
            inherit.aes = FALSE, size = 3)

print(p_blwt)

ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/MutationalPatterns/Figures/Group_comparisons/Relative_sign_contri_per_group_all_samples_20251024.pdf",
