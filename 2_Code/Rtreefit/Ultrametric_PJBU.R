...
tree = readRDS("~/hpc/pmc_vanboxtel/projects/Burkitt/3_Output/TreeBuilding_Alex/PJBU/Filtered_samples_v3/CPW_02/TreeObject0.2.RDS")
...
res_PJBU=fit_tree(tree=phylo_tree,switch_nodes = c(67),xcross = c(0.999), niter = 20000, cores = 4, model = "poisson_tree", early_growth_model_on = 1.0)

# Save ultrametric tree

save(res_PJBU, file = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/Rtreefit/Data/res_PJBU.RData")
load("~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/Rtreefit/Data/res_PJBU.RData")

# Create the plot
p <- ggtree(res_PJBU$ultratree, color = "grey") +
  theme_tree2() +
  scale_x_continuous(name = "Age (Years since conception)", limits = c(0, 9), breaks = 0:9) +
  # translucent grey box
  annotate(
    "rect",
    xmin = res_PJBU$upper_node_lims$lb95, xmax = res_PJBU$upper_node_lims$ub95,
    ymin = -Inf, ymax = Inf,
    fill = "grey", alpha = 0.2
  ) +
  ...
  geom_vline(xintercept = res_PJBU$upper_node_lims$mean, color = "black", linetype = "dotted") +
  theme(
...
# Save as PDF
ggsave(
  filename = "~/surfdrive/Shared/pmc_vanboxtel/projects/Burkitt_github/3_Output/Rtreefit/Figures/PJBU_ultrametric_tree_sbs1_blood_node67_0999.pdf",
  plot = p,
  width = 8,
  height = 6
)