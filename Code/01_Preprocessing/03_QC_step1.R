...
#ggsave(paste0("callable_loci_histogram_allsamples_", date, ".pdf"), width = 5, height = 3)
...
#write.csv(below_curve, file = "below_curve_samples.csv", row.names = F)
#write.csv(low_call_frac_df , file = "low_callable_loci.csv")
...
p1 <- ggplot(data = model_df_plot[model_df_plot$CNV != 'to do',], aes(x = CNV, y = residual, fill = CNV)) +
  geom_violin() + scale_fill_manual(values = qc_colors) + theme_burkitt() + 
