...
SBSs_PASS <- readRDS(file = "path/to/autosomal_PASS_variants_VAF015.RDS") # generated after running 01_MutLoad_sc_SBS.R
INDELs_PASS <- readRDS(file = "path/to/autosomal_INDEL_PASS_variants_VAF015.RDS") # generated after running 01_MutLoad_sc_SBS.R
...
below_curve_df <-  read.csv("path/to/below_curve_samples.csv") # generated after running 03_QC_step1
low_call_frac_df <-  read.csv("path/to/low_callable_loci.csv") # generated after running 03_QC_step1
bad_baf_df <- read.csv("path/to/bad_baf_samples.csv") # generated after running 03_QC_step1
...
ggplot(tvd_df, aes(x = TVD, y = reorder(samplename, TVD), 
  ...
  labs(title = "Total Variation Distance of VAF Distributions",
       x = "samplename", y = "TVD") +
  scale_fill_manual(values = c('#54BFB7','grey')) + theme() +
  theme(text =  element_text(size =  7, color = 'black'),
        axis.text = element_text(size = 2, colour = "black")) +
  ggTextAxisRotate()

#ggsave('TVD_ranked_flagged_3_mad.pdf', width = 10, height = 3)
...
write_csv(fail_df_pta, 'PTA_samples_failVAFcheck.txt')
