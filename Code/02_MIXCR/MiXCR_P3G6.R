...
# take only samples that start with P3G6 or PB11197 (same patient)
...
# Save file
#write.table(combined_df, "P3G6_combined_df.txt")

# Possibility to continue from saved df
#combined_df <- read.table("P3G6_combined_df.txt")
...
save_ht_const_rowheight(
  mat = mat_all,
  file = "Figures/P3G6_MiXCR_BCR_complete_heatmap_small.pdf",
  top_ha = top_ha,
  width_in = 4,
  row_mm = 2.2,
  extra_mm = 30,
...
save_ht_fixed_rowheight <- function(mat, top_ha, file, width_in = 4,
                                    row_mm = 2.2, extra_mm = 18) {
  nr <- nrow(mat)
  row_h <- unit(row_mm, "mm")
  hm_h  <- nr * row_h
  
  # Build heatmap object (NO sizing here)
  ht <- Heatmap(
    mat,
    na_col = "white",
    col = col_discrete,
  ...
  # Compute a PDF height that fits the constant row height (+ padding)
  pdf_h_in <- (convertUnit(hm_h, "mm", valueOnly = TRUE) + extra_mm) / 25.4
  
  pdf(file, width = width_in, height = pdf_h_in)
  draw(ht, heatmap_height = hm_h)   # ✅ THIS enforces identical row height
  dev.off()
}
top_ha <- HeatmapAnnotation(
  Sample = sample_groups,
...
pdf("Figures/P3G6_MiXCR_BCR_complete_heatmap_small.pdf", width=8, height=14)
par(mar = c(2, 2, 2, 2))
