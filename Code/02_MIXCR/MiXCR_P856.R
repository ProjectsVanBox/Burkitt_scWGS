...
# .tsv files can be found in Mendeley doi: 10.17632/phk5jhhm7d.1 --> MiXCR
# take only samples that start with P856 or PB14458 (same patient)
# make a list of paths and call it "files"
...
combined_df <- combined_df %>% 
  mutate(functional = case_when(grepl("_", aaSeqCDR3) ~ "No",
                                grepl("\\*", aaSeqCDR3) ~ "No", 
                                TRUE ~ "Yes"))
combined_df <- combined_df[combined_df$readCount != 1, ]

# Save file
#write.table(combined_df, "P856_combined_df.txt")

# Possibility to continue from saved df
#combined_df <- read.table("P856_combined_df.txt")
...
all_cells <- unique(c(rownames(igh_mat_cols), rownames(igk_mat_cols)))
...
# Now combine them
bcr_mat <- cbind(igh_mat_aligned, igk_mat_aligned)

# Add colours
cols <- distinctColorPalette(ncol(igh_mat_cols)+ncol(igk_mat_cols), runTsne = FALSE)
col_discrete <- colorRamp2(c(0:(length(cols)+1)), c("grey90", "white", cols))

# Keep only samples which are in bcr_mat
ordered_samples_in_mat <- ordered_samples[ordered_samples %in% rownames(bcr_mat)]

# Order samples
bcr_mat <- bcr_mat[ordered_samples_in_mat, ]
...
save_ht_const_rowheight <- function(mat, file, top_ha,
                                    width_in = 4,
                                    row_mm = 2.2,
                                    extra_mm = 30,
                                    ...) {
  ...
  nr <- nrow(mat)
  body_h <- unit(nr * row_mm, "mm")                 # heatmap BODY height
  pdf_h_in <- (nr * row_mm + extra_mm) / 25.4       # device height in inches
  
  ht <- Heatmap(
    mat,
    height = body_h,                                # ✅ key: constant row height
    top_annotation = top_ha,
    show_heatmap_legend = FALSE,
    ...
  )
  
  pdf(file, width = width_in, height = pdf_h_in, useDingbats = FALSE)
  draw(ht)                                          # no heatmap_body_height here
  dev.off()
}

mat_all <- t(bcr_mat)

save_ht_const_rowheight(
  mat = mat_all,
  file = "Figures/P856_MiXCR_BCR_complete_heatmap_small.pdf",
  top_ha = top_ha,
  width_in = 4,
  row_mm = 2.2,
  extra_mm = 30,
  ...
  cluster_columns=FALSE,
  show_column_names=TRUE,
  column_names_gp=gpar(col=column_names_color, fontsize=6),
  row_names_gp=gpar(fontsize=6),
  border=TRUE,
  column_title="P856 BCR recombinations",
  column_title_gp=gpar(fontsize=9),
  row_names_side="right",
  row_title_rot=0)

# Plot heatmap for all BCRs
pdf("Figures/P856_MiXCR_BCR_complete_heatmap_small.pdf", width=8, height=14)
par(mar = c(2, 2, 2, 2))
...
Heatmap(t(bcr_mat),
        ...
        cluster_columns = FALSE,
        show_column_names = TRUE,
        column_names_gp = gpar(col = column_names_color),
        border=TRUE,
        column_title= "P856 BCR recombinations",
        row_names_side = "right", 
        show_heatmap_legend = F,
...
#pdf("Figures/P856_MiXCR_BCR_heatmap.pdf", width=8, height=14)
...
#pdf("Figures/P856_MiXCR_BCR_heatmap.pdf_small.pdf", width = 3.5, height = 3.5)
...
Heatmap(
  ...
  cluster_columns = FALSE,
  show_column_names = TRUE,
  column_names_gp = gpar(fontsize = 4, col = column_names_color),  
