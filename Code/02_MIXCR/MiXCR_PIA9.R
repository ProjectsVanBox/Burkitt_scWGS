...
# take only samples that start with PIA9
...
# Save file
#write.table(combined_df, "PIA9_combined_df.txt")

# Possibility to continue from saved df
#combined_df <- read.table("PIA9_combined_df.txt")
...
save_ht_fixed_rowheight(
  mat = mat_all,
  top_ha = top_ha,
  file = "Figures/PIA9_MiXCR_BCR_complete_heatmap_small.pdf",
  width_in = 4,
  row_mm = 2.2,
  extra_mm = 22
)
...
pdf("Figures/PIA9_MiXCR_BCR_complete_heatmap_small.pdf", width=8, height=14)
