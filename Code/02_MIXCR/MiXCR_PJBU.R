...
# Description: Script for plotting MiXCR results for patient PJBU
...
# take only samples that start with PJBU
...
names(tsv_files_list) <- gsub("\\.tsv$", "", basename(PJBU_files))
...
# Save file
#write.table(combined_df, "~/Nextcloud/pmc_vanboxtel/projects/Burkitt_github/3_Output/MIXCR/Data/PJBU_combined_df.txt")

# Possibility to continue from saved df
#combined_df <- read.table("~/Nextcloud/pmc_vanboxtel/projects/Burkitt_github/3_Output/MIXCR/Data/PJBU_combined_df.txt")
...
Heatmap(t(igl_mat_cols),
        ...
        show_column_names = TRUE,
        border=TRUE,
        column_title= "PJBU IGL",
        row_names_side = "left", 
        top_annotation = top_ha,
        show_heatmap_legend = F,
        split=c(rep("IGL genes", 12), rep("CDR3aa", 16)))
...
save_ht_fixed_rowheight(
  mat = mat_all,
  top_ha = top_ha,
  file = "Figures/PJBU_MiXCR_BCR_complete_heatmap_small_Test.pdf",
  width_in = 4,
  row_mm = 2.2,
  extra_mm = 22
)
...
pdf("Figures/PJBU_MiXCR_BCR_complete_heatmap_small.pdf", width=8, height=14)
par(mar = c(2, 2, 2, 2))
...
Heatmap(t(bcr_mat),
        ...
        border=TRUE,
        column_title= "PJBU BCR recombinations",
...
#pdf("Figures/PJBU_MiXCR_BCR_heatmap.pdf", width=8, height=14)
...
#pdf("Figures/PJBU_MiXCR_BCR_heatmap.pdf_small.pdf", width = 3.5, height = 3.5)
