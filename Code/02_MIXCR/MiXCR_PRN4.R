...
# take only samples that start with PRN4 or PB08410 (same patient)
...
# Save file
#write.table(combined_df, "PRN4_combined_df.txt")

# Possibility to continue from saved df
#combined_df <- read.table("PRN4_combined_df.txt")
...
Heatmap(t(igl_mat_cols),
        ...
        show_column_names = TRUE,
        border=TRUE,
        column_title= "PRN4 IGL",
        row_names_side = "left", 
        top_annotation = top_ha,
        show_heatmap_legend = F,
        split=c(rep("IGL genes", n_bcr_igl), rep("CDR3aa", n_cdr3_igl)))
...
save_ht_fixed_rowheight(
  mat = mat_all,
  top_ha = top_ha,
  file = "Figures/PRN4_MiXCR_BCR_complete_heatmap_small.pdf",
  width_in = 4,
  row_mm = 2.2,
  extra_mm = 22
)
...
pdf("Figures/PRN4_MiXCR_BCR_complete_heatmap_small.pdf", width=3.5, height=3.5)
...
Heatmap(t(bcr_mat),
        ...
        border=TRUE,
        column_title= "PRN4 BCR recombinations",
...
#pdf("Figures/PRN4_MiXCR_BCR_heatmap.pdf", width=8, height=14)
...
#pdf("Figures/PRN4_MiXCR_BCR_heatmap.pdf_small.pdf", width = 3.5, height = 3.5)
Heatmap(
