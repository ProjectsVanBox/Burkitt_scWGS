...
# take only samples that start with PVA9
...
# Save file
#write.table(combined_df, "~/Nextcloud/pmc_vanboxtel/projects/Burkitt_github/3_Output/MIXCR/Data/PVA9_combined_df.txt")

# Possibility to continue from saved df
#combined_df <- read.table("~/Nextcloud/pmc_vanboxtel/projects/Burkitt_github/3_Output/MIXCR/Data/PVA9_combined_df.txt")
...
Heatmap(t(igl_mat_cols),
        ...
        top_annotation = top_ha,
        show_heatmap_legend = F,
        split=c(rep("IGL genes", 5), rep("CDR3aa",5)))
...
save_ht_fixed_rowheight(
  mat = mat_all,
  top_ha = top_ha,
  file = "Figures/PVA9_MiXCR_BCR_complete_heatmap_small_wo_single_reads.pdf",
  width_in = 4,
  row_mm = 2.2,
  extra_mm = 22
)
...
pdf("Figures/PVA9_MiXCR_BCR_complete_heatmap_small.pdf", width=8, height=14)
par(mar = c(2, 2, 2, 2))
