...
# Description: script to look at cellphy output of P856
...
# Obtain from Mendeley doi: 10.17632/phk5jhhm7d.1 --> CellPhy --> P856
...
# Save as PDF
ggsave(
  filename = "P856_absolute_tree_with_branch_info.pdf",
  plot = p1,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "P856_absolute_tree.pdf",
  plot = p2,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "P856_absolute_tree_with_bootstraps.pdf",
  plot = p2b,
  width = 8,
  height = 6
)
...
input_df_P856$Callable_Loci <- as.numeric(input_df_P856$Callable_Loci)

# Correct the branches
corrected_tree <- correct_branches(tree, input_df_P856)
...
# Save as PDF
ggsave(
  filename = "P856_absolute_tree_corrected_for_callable_loci.pdf",
  plot = p3,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "P856_absolute_tree_with_bootstraps_corrected_for_callable_loci.pdf",
  plot = p3b,
  width = 8,
  height = 6
)
...
saveRDS(branch_vcf, file = "branch_vcf_P856.rds")
branch_grl = convert_vcf_to_granges(branch_vcf_list = branch_vcf, ref_genome = ref_genome)
saveRDS(branch_grl, file = "branch_grl_P856.rds")
...
vcf_one <- vcf_bulk[, "PB14458-BLPL-BCELLBULK"]
...
bulk_vaf_matrix <- matrix(VAF,
                          ncol = 1,
                          dimnames = list(names(VAF), "PB14458-BLPL-BCELLBULK"))
bulk_vafs_PL <- as.numeric(bulk_vaf_matrix[, 1]) 

# Create a named vector of VAFs for lookup
names(bulk_vafs_PL) <- bulk_keys
bulk_vafs_PL <- bulk_vafs_PL[ bulk_vafs_PL != 0 ]
...
p4a <- ggplot(plot_df, aes(x = VAF), drop = FALSE) + 
...
# Save as PDF
ggsave(
  filename = "P856_bulk_VAF_distribution_histogram_PL_haplotypecaller.pdf",
  plot = p4a,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "P856_bulk_VAF_distribution_violin_PL_haplotypecaller.pdf",
  plot = p5a,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "P856_bulk_VAF_distribution_intermediate_branches_violin_PL_haplotypecaller.pdf",
  plot = p6a,
  width = 8,
  height = 6
)
...
# Get variant keys and VAFs for the bulk
bulk_keys <- variant_key(vcf_bulk)
vcf_one <- vcf_bulk[, "PB14458-BLBM-BCELLBULK"]
ad <- geno(vcf_one)$AD   # still a matrix: rows = variants, 1 column
...
bulk_vaf_matrix <- matrix(VAF,
                          ncol = 1,
                          dimnames = list(names(VAF), "PB14458-BLBM-BCELLBULK"))
bulk_vafs_BM <- as.numeric(bulk_vaf_matrix[, 1]) 

# Create a named vector of VAFs for lookup
names(bulk_vafs_BM) <- bulk_keys
bulk_vafs_BM <- bulk_vafs_BM[ bulk_vafs_BM != 0 ]
...
# Save as PDF
ggsave(
  filename = "P856_bulk_VAF_distribution_histogram_BM_haplotypecaller.pdf",
  plot = p4b,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "P856_bulk_VAF_distribution_violin_BM_haplotypecaller.pdf",
  plot = p5b,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "P856_bulk_VAF_distribution_intermediate_branches_violin_BM_haplotypecaller.pdf",
  plot = p6b,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "P856_reconstructed_cosine_per_branch.pdf",
  plot = p7,
  width = 8,
  height = 6
)

write.csv(contribution, file = "P856_per_branch_sbs_contribution.csv", row.names = TRUE)
...
# Save as PDF
ggsave(
  filename = "P856_signatures_in_tree.pdf",
  plot = p9,
  width = 8,
  height = 6
)
...
write.csv(contribution_matrix, "sbs_contr_per_group_P856.csv", row.names = T)

p10 <- plot_contribution(contribution_matrix[,], palette = sig_cols) 

# Save as PDF
ggsave(
  filename = "~P856_signatures_per_group.pdf",
  plot = p10,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "P856_signatures_per_group_separate_bars.pdf",
  plot = p11,
  width = 8,
  height = 6
)