...
# Description: script to look at cellphy output for PJBU
...
# Obtain from Mendeley doi: 10.17632/phk5jhhm7d.1 --> CellPhy --> PJBU
tree = readRDS("TreeObject0.1.RDS") 
vcf = VariantAnnotation::readVcf("PJBU.vep.sub.SNV.autosomal.noBulksNoBlacklist.vcf")
vcf_bulk = VariantAnnotation::readVcf("PJBU.vep.sub.SNV.autosomal.vcf")
...
# Save as PDF
ggsave(
  filename = "PJBU_absolute_tree_with_branch_info.pdf",
  plot = p1,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "PJBU_absolute_tree.pdf",
  plot = p2,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "PJBU_absolute_tree_with_bootstraps.pdf",
  plot = p2b,
  width = 8,
  height = 6
)
...
input_df_PJBU$Callable_Loci <- as.numeric(input_df_PJBU$Callable_Loci)

# Correct the branches
corrected_tree <- correct_branches(tree, input_df_PJBU)
...
# Save as PDF
ggsave(
  filename = "PJBU_absolute_tree_corrected_for_callable_loci.pdf",
  plot = p3,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "PJBU_absolute_tree_with_bootstraps_corrected_for_callable_loci.pdf",
  plot = p3b,
  width = 8,
  height = 6
)
...
saveRDS(branch_vcf, file = "branch_vcf_PJBU.rds")
branch_grl = convert_vcf_to_granges(branch_vcf_list = branch_vcf, ref_genome = ref_genome)
saveRDS(branch_grl, file = "branch_grl_PJBU.rds")
...
bulk_vaf_matrix <- matrix(VAF,
                          ncol = 1,
                          dimnames = list(names(VAF), "PJBUGBDABC82"))
...
# Save as PDF
ggsave(
  filename = "PJBU_bulk_VAF_distribution_histogram_haplotypecaller.pdf",
  plot = p4,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "PJBU_bulk_VAF_distribution_violin_haplotypecaller.pdf",
  plot = p5,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "PJBU_bulk_VAF_distribution_intermediate_branches_violin_haplotypecaller.pdf",
  plot = p6,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "PJBU_reconstructed_cosine_per_branch.pdf",
  plot = p7,
  width = 8,
  height = 6
)

write.csv(contribution, file = "PJBU_per_branch_sbs_contribution.csv", row.names = TRUE)
...
# Save as PDF
ggsave(
  filename = "PJBU_signatures_in_tree.pdf",
  plot = p9,
  width = 8,
  height = 6
)
...
write.csv(contribution_matrix, "sbs_contr_per_group_PJBU.csv", row.names = T)

p10 <- plot_contribution(contribution_matrix[,], palette = sig_cols) 

# Save as PDF
ggsave(
  filename = "PJBU_signatures_per_group.pdf",
  plot = p10,
  width = 8,
  height = 6
)
...
# Save as PDF
ggsave(
  filename = "PJBU_signatures_per_group_separate_bars.pdf",
  plot = p11,
  width = 8,
  height = 6
)