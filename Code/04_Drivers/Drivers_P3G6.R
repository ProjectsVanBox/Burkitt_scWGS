...
setwd("~/Nextcloud/pmc_vanboxtel/projects/Burkitt_github/3_Output/Drivers/")
...
P3G6_samples <- c(
  ...
  "P3G6GPDABC26", "PB11197-BLASC-BCELLBULK"
)
...
# obtain vcfs from Mendeley doi: 10.17632/phk5jhhm7d.1 --> Drivers_single_cell --> P3G6
...
bulk_sample <- bulk_samples[bulk_samples$Sample == "P3G6_bulk", ]
bulk_sample <- bulk_sample %>%
  dplyr::rename(sample = Sample) %>%                 # 1) Sample -> sample
  mutate(sample = "PB11197-BLASC-BCELLBULK") %>%            #    set all values
...
# save combined_data 
write.csv(combined_data, "combined_data_P3G6.csv")
...
oncoplot_matrix <- oncoplot_matrix %>%
  dplyr::select(-any_of(P3G6_normal))

# Reorder columns: driver first, then P3G6_samples
oncoplot_matrix <- oncoplot_matrix %>%
  dplyr::select(driver, all_of(P3G6_tumour))
...
P3G6_rows <- P3G6_rows %>%
  mutate(
    Category = case_when(
      ...
      Sample_name == "PB11197-BLASC-BCELLBULK" ~ "Bulk Tumour",
...
# Plot
pdf("P3G6_oncoprint_rebuttal.pdf",
    width = 10, height = 6)
