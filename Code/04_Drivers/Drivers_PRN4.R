...
low_callable_df<- read.csv('~/Nextcloud/pmc_vanboxtel/projects/Burkitt_github/3_Output/QC/Data/low_callable_loci.csv')
below_curve_df <- read.csv('~/Nextcloud/pmc_vanboxtel/projects/Burkitt_github/3_Output/QC/Data/below_curve_samples.csv')
bad_baf_df     <- read.csv('~/Nextcloud/pmc_vanboxtel/projects/Burkitt_github/3_Output/QC/Data/bad_baf_samples.csv')
fail_vaf_df    <- read.csv('~/Nextcloud/pmc_vanboxtel/projects/Burkitt_github/3_Output/QC/Data/PTA_samples_failVAFcheck.txt')
...
# obtain vcfs from Mendeley doi: 10.17632/phk5jhhm7d.1 --> Drivers_single_cell --> PRN4
...
bulk_sample <- bulk_samples[bulk_samples$Sample %in% c("PRN4_bulk", "PMCID163AAN"), ]
bulk_sample <- bulk_sample %>%
  dplyr::rename(sample = Sample) %>%                 # rename column
  mutate(sample = ifelse(sample == "PRN4_bulk", "PRN4GBDLBC72", sample)) %>% 
...
# save combined_data 
write.csv(combined_data, "combined_data_PRN4.csv")
...
# Ensure all PRN4_samples exist as columns (add NA if missing)

for (s in PRN4_samples) {
...
oncoplot_matrix <- oncoplot_matrix %>%
  dplyr::select(-any_of(PRN4_normal))

# Reorder columns: driver first, then PRN4_samples
oncoplot_matrix <- oncoplot_matrix %>%
  dplyr::select(driver, all_of(PRN4_tumour))
...
PRN4_samples_filtered <- setdiff(PRN4_samples, blacklist)
PRN4_samples_filtered <- intersect(PRN4_samples_filtered, colnames(effect_matrix_after_check_T))
...
# Categorize samples in PRN4_rows
PRN4_rows <- PRN4_rows %>%
  mutate(
    Category = case_when(
      Sample_name %in% normal_bcell_samples ~ "Normal B-cell",
      Sample_name == "PRN4GBDLBC72" ~ "Bulk Tumour",
...
sample_to_cat <- setNames(as.character(PRN4_rows$Category), PRN4_rows$Sample_name)
...
# Plot
pdf("PRN4_oncoprint_rebuttal.pdf",
    width = 10, height = 6)
