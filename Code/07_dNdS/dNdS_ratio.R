...
# Load RDS lists
branch_vcf_P3G6 <- readRDS("branch_vcf_P3G6.rds") # generated when running CellPhy_Tree_P3G6.R script
branch_vcf_PVA9 <- readRDS("branch_vcf_PVA9.rds") # generated when running CellPhy_Tree_PVA9.R script
branch_vcf_PIA9 <- readRDS("branch_vcf_PIA9.rds") # generated when running CellPhy_Tree_PIA9.R script
branch_vcf_PJBU <- readRDS("branch_vcf_PJBU.rds") # generated when running CellPhy_Tree_PJBU.R script
branch_vcf_P856 <- readRDS("branch_vcf_P856.rds") # generated when running CellPhy_Tree_P856.R script
branch_vcf_PRN4 <- readRDS("branch_vcf_PRN4.rds") # generated when running CellPhy_Tree_PRN4.R script

# Define branches per sample (again, taken when running CellPhy_Tree_X.R scripts)
...
# BL-private essential vs control (DepMap top500 per BL line)
...
# Inputs
files <- c(
  "Data/gene_effects_TL1.csv",
  "Data/gene_effects_BL70.csv",
  "Data/gene_effects_BLUE1.csv",
  "Data/gene_effects_ST486.csv",
  "Data/gene_effects_NAMALWA.csv"
)

# Helper: count coding substitutions in a gene set for a given fitted group
count_coding_in_set <- function(group, genes) {
  fit_by_group[[group]]$annotmuts %>%
    dplyr::filter(gene %in% genes) %>%
    dplyr::filter(impact %in% c("Missense","Nonsense","Essential_Splice","Synonymous")) %>%
    nrow()
}

## Read top 500 genes per file
gene_lists <- lapply(files, function(f) {
  df <- read_csv(f, show_col_types = FALSE)
  
  if (!("gene" %in% names(df))) stop("No 'gene' column in: ", f)
  
  df %>%
    dplyr::filter(!is.na(gene), gene != "") %>%
    # dplyr::arrange(gene_effect) %>%  # uncomment if needed (most negative = most essential)
    dplyr::pull(gene) %>%
    head(500)
})

bl_essential_combined <- unique(unlist(gene_lists))
bl_essential_combined <- unique(bl_essential_combined[!is.na(bl_essential_combined) & bl_essential_combined != ""])

length(bl_essential_combined)

# Restrict to genes in dndscv RefCDS universe for BL-Private fit
fit_blp <- fit_by_group[["BL-Private"]]
gene_universe <- unique(fit_blp$genemuts$gene_name)

bl_ess <- intersect(bl_essential_combined, gene_universe)
length(bl_ess)

# Build a matched control set using BL-Private expected coding opportunity
gm <- fit_blp$genemuts %>%
  dplyr::mutate(
    exp_total = exp_syn + exp_mis + exp_non + exp_spl
  ) %>%
  dplyr::select(gene_name, exp_syn, exp_mis, exp_non, exp_spl, exp_total)

candidates <- setdiff(gene_universe, bl_ess)

n_bins <- 20
gm_bins <- gm %>%
  dplyr::mutate(bin = dplyr::ntile(exp_total, n_bins))

ess_df  <- gm_bins %>% dplyr::filter(gene_name %in% bl_ess)
cand_df <- gm_bins %>% dplyr::filter(gene_name %in% candidates)

set.seed(1)

controls <- purrr::map_dfr(split(ess_df, ess_df$bin), function(ess_bin) {
  b <- unique(ess_bin$bin)
  k <- nrow(ess_bin)
  
  pool <- cand_df %>% dplyr::filter(bin == b)
  
  # broaden if a bin is sparse
  if (nrow(pool) < k) {
    pool <- cand_df %>% dplyr::filter(bin %in% c(b-1, b, b+1))
  }
  if (nrow(pool) < k) {
    stop("Not enough candidate genes to match bin ", b,
         " (need ", k, ", have ", nrow(pool),
         "). Try n_bins=10 or expand candidate universe.")
  }
  
  pool %>% dplyr::slice_sample(n = k)
})

bl_ctl <- unique(controls$gene_name)
length(bl_ctl)  # should equal length(bl_ess)

# Quick match check (expected opportunity)
check_df <- dplyr::bind_rows(
  gm_bins %>% dplyr::filter(gene_name %in% bl_ess) %>% dplyr::mutate(Set="Essential"),
  gm_bins %>% dplyr::filter(gene_name %in% bl_ctl) %>% dplyr::mutate(Set="Control")
)
...
check_df %>%
  ...
  dplyr::summarise(
    n = dplyr::n(),
    mean_exp_total = mean(exp_total),
    median_exp_total = median(exp_total),
    mean_exp_syn = mean(exp_syn),
    median_exp_syn = median(exp_syn),
    .groups = "drop"
  )

# BL-Private mutation counts in each set (sanity)
c(
  Essential = count_coding_in_set("BL-Private", bl_ess),
  Control   = count_coding_in_set("BL-Private", bl_ctl)
)

# Compute set-level omega from BL-Private genemuts (stable)
compute_set_omega <- function(fit_obj, gene_set, pseudocount = 0.5) {
  
  gm2 <- fit_obj$genemuts %>%
    dplyr::filter(gene_name %in% gene_set)
  
  n_syn <- sum(gm2$n_syn)
  n_nonsyn <- sum(gm2$n_mis + gm2$n_non + gm2$n_spl)
  
  exp_syn <- sum(gm2$exp_syn)
  exp_nonsyn <- sum(gm2$exp_mis + gm2$exp_non + gm2$exp_spl)
  
  dN <- (n_nonsyn + pseudocount) / (exp_nonsyn + pseudocount)
  dS <- (n_syn    + pseudocount) / (exp_syn    + pseudocount)
  ...
  data.frame(
    ...
    n_nonsyn = n_nonsyn,
    exp_syn = exp_syn,
    exp_nonsyn = exp_nonsyn
  )
}

omega_ess <- compute_set_omega(fit_blp, bl_ess)
omega_ctl <- compute_set_omega(fit_blp, bl_ctl)

omega_ess
omega_ctl

# Gene-bootstrap CIs in BL-Private
set.seed(1)

gm_boot <- fit_blp$genemuts %>%
  dplyr::mutate(
    n_nonsyn = n_mis + n_non + n_spl,
    exp_nonsyn = exp_mis + exp_non + exp_spl
  ) %>%
  dplyr::select(gene_name, n_syn, n_nonsyn, exp_syn, exp_nonsyn)

gm_ess <- gm_boot %>% dplyr::filter(gene_name %in% bl_ess)
gm_ctl <- gm_boot %>% dplyr::filter(gene_name %in% bl_ctl)

omega_from_genes <- function(gm_sub, idx, pseudocount = 0.5) {
  x <- gm_sub[idx, , drop = FALSE]
  n_syn <- sum(x$n_syn)
  n_nonsyn <- sum(x$n_nonsyn)
  exp_syn <- sum(x$exp_syn)
  exp_nonsyn <- sum(x$exp_nonsyn)
...
for (b in seq_len(B)) {
  idxE <- sample.int(nE, size = nE, replace = TRUE)
  idxC <- sample.int(nC, size = nC, replace = TRUE)
  boot_ess[b] <- omega_from_genes(gm_ess, idxE, pseudocount)
  boot_ctl[b] <- omega_from_genes(gm_ctl, idxC, pseudocount)
}

boot_diff <- boot_ctl - boot_ess  # positive => control > essential

ci_ess  <- quantile(boot_ess,  probs = c(0.025, 0.5, 0.975), na.rm = TRUE)
ci_ctl  <- quantile(boot_ctl,  probs = c(0.025, 0.5, 0.975), na.rm = TRUE)
ci_diff <- quantile(boot_diff, probs = c(0.025, 0.5, 0.975), na.rm = TRUE)

p_one_sided <- mean(boot_diff <= 0, na.rm = TRUE)  # H1: essential < control

list(
  genes = data.frame(n_essential = length(bl_ess), n_control = length(bl_ctl)),
  coding_counts = c(
    Essential = count_coding_in_set("BL-Private", bl_ess),
    Control   = count_coding_in_set("BL-Private", bl_ctl)
  ),
  point_estimates = data.frame(
    omega_ess = omega_ess$omega,
    omega_ctl = omega_ctl$omega,
    diff_ctl_minus_ess = omega_ctl$omega - omega_ess$omega
  ),
  bootstrap_CI = list(
    essential = ci_ess,
    control = ci_ctl,
    diff_ctl_minus_ess = ci_diff
  ),
  p_one_sided_ess_less_than_ctl = p_one_sided
)
