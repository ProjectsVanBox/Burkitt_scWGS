...
sbsblood <- read.table("Data/sigfit_cosmic3_bloodsig_Aug2020.txt", sep = "\t", header = TRUE) |> as.matrix()
...
# Save to CSV
write.csv(df_sbs9_status, "Normal_SBS9_status_table.csv", row.names = FALSE)
...
ggsave(
  filename = "SBS_high_vs_low_SNV_load_comparison_WT_only_all_samples_20251003.pdf",
...
ggsave(
  filename = "WT_ageline_per_sig_and_ctgpg_all_samples.pdf",
...
# load blwt_files : these are the sbs_contr_per_group_.*.csv files generated after running the 03_CellPhy scripts
...
ggsave(
  filename = "Relative_sign_contri_per_group_all_samples.pdf",
