...
tree = readRDS("TreeObject0.1.RDS") # Obtain from Mendeley doi: 10.17632/phk5jhhm7d.1 --> CellPhy --> P3G6
...
# VCF per branch
P3G6_contribution <- read.csv("P3G6_per_branch_sbs_contribution.csv", row.names = 1) # generated after running CellPhy_Tree_P3G6.R script
...
agedf <- data.frame(
  ...
  age = rep(14.7,32) # 13.8 + 0.9 (so post-conception)
)
phylo_tree$agedf<-agedf
...
save(res_P3G6, file = "res_P3G6_rebuttal.RData")
#load("res_P3G6_rebuttal.RData")
...
# Save as PDF
ggsave(
  filename = "P3G6_ultrametric_tree_sbs1_blood_node24_0999_rebuttal.pdf",
  plot = p_P3G6,
  width = 8,
  height = 6
)
...
# Save ultrametric tree
#save(res_P3G6, file = "res_P3G6.RData")
#load("res_P3G6.RData")
...
# Save as PDF
ggsave(
  filename = "P3G6_ultrametric_tree_sbs1_blood_node24_0999_wo_P3G6GPDABC31.pdf",
  plot = p_P3G6_31,
  width = 8,
  height = 6
)
