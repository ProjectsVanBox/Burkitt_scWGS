...
tree = readRDS("TreeObject0.1.RDS") # Obtain from Mendeley doi: 10.17632/phk5jhhm7d.1 --> CellPhy --> PVA9
...
# VCF per branch
PVA9_contribution <- read.csv("PVA9_per_branch_sbs_contribution.csv", row.names = 1) # generated after running CellPhy_Tree_PVA9.R script
...
# Save as PDF
ggsave(
  filename = "PVA9_ultrametric_tree_sbs1_blood_node72_0999_wo_PVA9GTDBBC74.pdf",
  plot = p74,
  width = 8,
  height = 6
)

# Test another random normal B-cell

tips_to_remove <- c("PVA9GTDBBC65") #remove one normal B-cell

phylo_tree_subset_65 <- drop.tip(phylo_tree, tips_to_remove )

res_PVA9_65=fit_tree(tree=phylo_tree_subset_65,switch_nodes = c(72),xcross = c(0.999), niter = 20000, cores = 12, model = "poisson_tree", early_growth_model_on = 1.0)

# Create the plot
p65 <- ggtree(res_PVA9_65$ultratree, color = "grey") +
  theme_tree2() +
  #geom_tiplab()+
  scale_x_continuous(name = "Age (Years since conception)", limits = c(0, 14), breaks = 0:14) +
  # translucent grey box
  annotate(
    "rect",
    xmin = res_PVA9_65$upper_node_lims$lb95, xmax = res_PVA9_65$upper_node_lims$ub95,
    ymin = -Inf, ymax = Inf,
    fill = "grey", alpha = 0.2
  ) +
  # dotted vertical lines at x=0 and x=4.1
  ...
  geom_vline(xintercept = 0.9, color = "blue", linetype = "dotted") +
  geom_vline(xintercept = 13.6, color = "red", linetype = "dotted") +
  geom_vline(xintercept = res_PVA9_65$upper_node_lims$mean, color = "black", linetype = "dotted") +
  theme(
    axis.text.x = element_text(size = 10, angle = 90, vjust = 0.5, hjust = 1),
    axis.title.x = element_text(size = 12, angle = 180, vjust = 0.5)
  )
...
res_PVA9_65$upper_node_lims$mean
res_PVA9_65$upper_node_lims$ub95
res_PVA9_65$upper_node_lims$lb95

# Save as PDF
ggsave(
  filename = "PVA9_ultrametric_tree_sbs1_blood_node72_0999_wo_PVA9GTDBBC65.pdf",
  plot = p65,
  width = 8,
  height = 6
)
